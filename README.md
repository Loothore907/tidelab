...
- [Normalized-package / LEAN synthetic execution parity](docs/evidence/TL-003-PACKAGE-LEAN-PARITY.md)
...
- [TL-002 split-source recovery gap probe](docs/evidence/TL-002-KRAKEN-SPLIT-SOURCE-RECOVERY.md)
...
A bounded private audit and preregistered development screen have run on the five-market local database. Read `data/okx/first-pass-coverage-private.json` for coverage and `data/tl003_screen_v1/development-decision.json` plus its append-only registry for the screen decision. These real-data-derived records stay ignored and private; no market ranking or metric is published here.
...
- [Shared historical batch backend contract](docs/decisions/TL-003-HISTORICAL-BATCH-BACKEND.md): next synthetic implementation, reuse boundaries, admission/recovery and acceptance; not an implemented historical runner or real-data authorization.
