...
Exit zero means the full comparison matched. An unsupported input, runtime failure or mismatch exits nonzero and retains its reason. Reusing an output directory is rejected. The result is `result.json`; inspect `python.json`, `lean.json`, `build.log` and `engine.log` for the evidence. `attempt.json` binds the contract, TideLab head/dirty state, actual runtime-source hashes and cost identity; `identities.json` binds retained exact input bytes, and `runtime.json` records the SDK, LEAN pin and engine-input hash. Accepted decimal text is canonicalized to invariant fixed-point form for the C# wire input without changing its value; the original package and fixture bytes remain intact. A started directory without a result is an incomplete attempt, not a passing run. These are synthetic attempt artifacts, not the real research trial registry. Do not delete failed attempts to obtain a clean result.
...
Local verification on 2026-09-26: the focused Python 3.12.13 / .NET 10.0.401 WSL run passed all 14 tests, including all six real LEAN integration cases. The Windows suite passed 123 tests, with the optional Nautilus test and six explicitly environment-gated LEAN cases skipped; context tests passed 2/2. The LEAN acceptance comes from the separate runtime run, not those skips. Exact-head CI and review results are recorded in the owning PR.
...
Distinct review of `804731b` reproduced a supported decimal-text mismatch: `2E+2` passed Python validation but failed C# parsing. The corrected adapter canonicalizes validated numeric text; an actual LEAN regression case covers exponent-form prices, numeric constants and target fraction, plus separators and Unicode digits. The original failed reviewer attempt remains under ignored `artifacts/reviewer-804731b-exponent/`. Review disposition and the corrected reviewed SHA are recorded in the PR.
