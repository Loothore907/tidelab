...
For a TradingView source, save the exact published Pine text manually as a UTF-8 file under ignored `data/strategy_intake/`. Use `python scripts/strategy_intake.py capture-pine RECORD SOURCE_FILE` while the record is `captured`. The command retains the original bytes and a provenance receipt under ignored `data/strategy_intake/pine_sources/`; it prints the SHA-256 and `sha256:<digest>` revision to put in a **new** intake record version. Do not edit the registered version. `python scripts/strategy_intake.py verify-pine NEW_RECORD` checks the retained bytes, receipt, candidate ID and locator. Registration of a `specified` or `implementation_selected` TradingView record also requires that check. The tool cannot establish that a manually supplied file really came from the cited publication, that TradingView will keep serving it, or that reuse rights are clear. Compare the visible publication and any version history yourself; retain source privately and leave unresolved semantics in `captured`.
