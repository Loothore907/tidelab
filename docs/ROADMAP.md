...
Current state: TL-001 is integrated, and [TL-001A selected pinned LEAN](decisions/TL-001A-INITIAL-ENGINE-SELECTION.md) for the first synthetic historical/local-paper integration. TL-001B has [selected OKX archives for local historical research](evidence/TL-001B-OKX-HISTORICAL-20260924.md) but remains open for timely forward data and optional public artifacts. TL-002 has a [durable synthetic paper-intent barrier](evidence/TL-002-PAPER-INTENT-BARRIER.md), [same-database order recovery check](evidence/TL-002-SYNTHETIC-LOCAL-RECONCILIATION.md), [one-order LEAN local report/account join](evidence/TL-002-LEAN-SYNTHETIC-INTENT-JOIN.md), [one forward sell with a two-order ledger](evidence/TL-002-LEAN-FORWARD-SELL-LEDGER.md), [pinned LEAN synthetic H1 report-to-next-exit handoff](evidence/TL-002-H1-LEAN-REPORT-HANDOFF.md), [costed synthetic execution/trial metrics](evidence/TL-002-H1-CONSERVATIVE-EXECUTION-TRIAL-ACCOUNTING.md), and a [private, registered H1 development runner](evidence/TL-002-H1-PRIVATE-DEVELOPMENT-RUNNER.md). The local development and one validation attempt underwent [independent private accounting and decision review](evidence/TL-002-H1-PRIVATE-REVIEW-VALIDATION-GATE.md). The gate record stays ignored; keep its outcome private, do not retune or promote frozen H1 v1, and leave its untouched partition closed. External broker finality and packaging remain open under issue #48.
...
The [post-handoff correction check](evidence/TL-002-H1-POST-HANDOFF-CORRECTION.md) guards the successor claim with fresh synthetic report and snapshot reads. A later observed change latches a durable hold before another LEAN order route; no correction reversal or external broker finality is claimed.

TL-003 preparation has a [synthetic first-pass universe coverage gate](evidence/TL-003-FIRST-PASS-COVERAGE-GATE.md). It audits the fixed five-asset plan's exact-source hours, archive provenance and shared 24-month continuity without reading prices or ranking strategies. A later, bounded private acquisition/import and audit covered the other four markets; exact results and gaps remain in ignored local `data/okx/`. This does not clear #3's timely-feed gate or establish a promising market-and-strategy candidate.

[Issue #86](https://github.com/Loothore907/tidelab/issues/86) owns the [small TL-003 v1 candidate screen](experiments/FIVE-MARKET-SCREEN-V1-PREREGISTRATION.md). Ten fixed development combinations, one selected validation and at most one untouched check are independent of H1 v1. An offline survivor still needs pinned LEAN parity, deeper execution evidence and a timely rights-cleared source before any forward-paper claim.

[Issue #89](https://github.com/Loothore907/tidelab/issues/89) adds a source-backed intake foundation for external strategy ideas. The [drift review and pre-code checkpoint](decisions/TL-003-STRATEGY-INTAKE-BOUNDARY.md) separate selecting an exact candidate from implementing its rule and from authorizing a market-data trial. The intake registry is metadata, not an executable strategy interface or a completed multi-source import pipeline.
...
Publication standard approved 2026-09-21 and priority clarified 2026-09-22: publish TideLab code and synthetic fixtures; prioritize usable local research and forward-paper data. Public raw datasets and real-data-derived charts/results/video are optional and require separate artifact-specific rights. Prefer a reproducible acquisition recipe where allowed. The 2026-09-24 owner clarification makes candidate discovery across markets a TideLab research responsibility; the owner is not selecting pairs. Use affordable comparable breadth to screen an objectively defined universe, then obtain deeper data for shortlisted questions. BTC/OKX and frozen H1 are initial system/baseline fixtures, not the product boundary or a selected opportunity. Initial data-source spend remains $0. A subscription needs a measured gap, confirmed rights/coverage, effective cost cap, and separate purchase decision; a roughly $70-$80 monthly bill is not justified by the current incomplete foundation alone. See the [TL-001B preflight](evidence/TL-001B-DATA-RIGHTS-PREFLIGHT.md), [source option map](evidence/TL-001B-DATA-OPTIONS-20260924.md), and [OKX historical selection](evidence/TL-001B-OKX-HISTORICAL-20260924.md). The historical research source passed a local coverage/import gate; an on-time hourly forward-paper source has not.
...
| TL-003 | Reproducible research and candidate discovery | Immutable experiment registry, deterministic replay/backtester, trial accounting, chronological partitions, benchmarks, cost stress, registered baseline strategy family including frozen H1, and an objective, rights-cleared multi-market screening process that records selected and rejected candidates |
...
Build TL-003 as the [deterministic strategy pipeline](decisions/TL-003-DETERMINISTIC-STRATEGY-PIPELINE.md): source-specific ingestion/parsing, typed versioned packages, capability matching, batch testing, complete variant accounting and a shared evaluation backend. The normalized-JSON/synthetic checker, bounded Pine v5 frontend and synthetic attempt ledger are foundation proofs, not a general paper/Pine parser or a historical batch engine. A [bounded package-to-LEAN comparison](evidence/TL-003-PACKAGE-LEAN-PARITY.md) exercises synthetic orders, fills and portfolio accounting through shared package inputs; its harness does not establish feed or venue realism. PR #103 integrated the shared historical batch backend. The separately approved [RSI private slice](evidence/TL-003-RSI-PRIVATE-BATCH.md) exercises a selected candidate through that route; measured source support remains a later decision. Keep candidate selection and real-data trial authorization separate. AI may draft a translation for review but is not the default evaluator or a runtime dependency.
...
Candidate-market discovery is also a selection process. Define the eligible universe and screening rules before ranking markets; record exclusions, missing/changed symbols, data gaps, screening dates, and every market-strategy combination examined. A broad cheap screen nominates candidates for deeper research; it does not establish profitability. Obtain finer bars, trades, or quotes only when the shortlisted strategy and execution question require them, and verify each added source's research and retention rights. A private five-market coverage audit and one fixed offline screen exist. They do not establish a general candidate scanner or validate cross-market strategy selection.
