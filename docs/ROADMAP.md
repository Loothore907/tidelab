...
Current state: TL-001 is integrated, and [TL-001A selected pinned LEAN](decisions/TL-001A-INITIAL-ENGINE-SELECTION.md) for the first synthetic historical/local-paper integration. [TL-001B selected a bounded Bitstamp data source](decisions/TL-001B-INITIAL-RESEARCH-DATA.md) for local noncommercial research, with private historical and recent hourly samples; publication/commercial use and unattended forward collection remain separate gates. TL-002 has a [durable synthetic paper-intent barrier](evidence/TL-002-PAPER-INTENT-BARRIER.md), [same-database order recovery check](evidence/TL-002-SYNTHETIC-LOCAL-RECONCILIATION.md), [one-order LEAN local report/account join](evidence/TL-002-LEAN-SYNTHETIC-INTENT-JOIN.md), and [one forward sell with a two-order ledger](evidence/TL-002-LEAN-FORWARD-SELL-LEDGER.md). General runtime reconciliation, strategy integration, and packaging remain open under issue #48.

Data-depth and rights finding: a 2026-09-16 public Coinbase 30-day probe admitted only the latest 349 of 720 requested hourly bars after rejecting responses outside each requested window. Coinbase's Market Data Terms reviewed 2026-09-21 also create material automated-analysis and publication-rights questions for market data and derived artifacts. Continue to pause new Coinbase acquisition for strategy development or automated analysis. The selected Bitstamp source supports a narrow local noncommercial research interpretation and passed a historical/recent continuity check; do not extend that interpretation to public artifacts or commercial use.
