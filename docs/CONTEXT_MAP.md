...
- [TL-001A decision](decisions/TL-001A-INITIAL-ENGINE-SELECTION.md) / [issue #2](https://github.com/Loothore907/tidelab/issues/2): pinned LEAN is selected for the first synthetic historical/local-paper integration. The joined synthetic selection gate and H1 parity probes support the bounded choice; cross-process paper authority, forward sell reconciliation, packaging, and actual research-data rights remain open. Start at the decision, then `docs/evidence/TL-001A-LEAN-SELECTION-GATE.md` and `scripts/lean_fallback/`.
