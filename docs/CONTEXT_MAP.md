...
- [Issue #2](https://github.com/Loothore907/tidelab/issues/2) / [merged PR #7](https://github.com/Loothore907/tidelab/pull/7): TL-001A synthetic NautilusTrader checkpoint. A Redis-backed native sandbox node restored one denied closed order after abrupt exit, but pinned 2.0.0rc5 rejects backing with an external Python data client. Pending-order/account recovery, reconciliation, and adoption remain open; a bounded LEAN fallback screen is in `docs/evidence/TL-001A-LEAN-FALLBACK-PREFLIGHT.md`. See also `docs/evidence/TL-001A-ENGINE-BAKEOFF.md` and `scripts/tl001a_backed_cache_probe.py`.
