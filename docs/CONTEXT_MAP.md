...
- [Issue #2](https://github.com/Loothore907/tidelab/issues/2): TL-001A engine evaluation. Pinned NautilusTrader 2.0.0rc5 rejects backing with an external Python data client. LEAN has synthetic historical/forward H1 strategy and risk parity, pending-order setup, and controlled running-engine delivery recovery. A single-writer paper source and durable-intent probe recover controlled process-loss windows by stable client ID. Fresh LEAN setup matches the accepted intent, three broker open orders, corrected journal, cash and holding; a wrong broker ID blocks. This is a static synthetic restart join, not an operational next-order gate. A new cost/fill probe shows pinned LEAN fills a one-unit equity market order against only 0.4 displayed ask size; model realism remains open. Neither engine is adopted. Start at `docs/evidence/TL-001A-ENGINE-DECISION-MAP.md`, then `docs/evidence/TL-001A-LEAN-COST-FILL-GATE.md` and `scripts/lean_fallback/`.
