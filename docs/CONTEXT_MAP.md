...
- [Issue #2](https://github.com/Loothore907/tidelab/issues/2): TL-001A engine evaluation. Pinned NautilusTrader 2.0.0rc5 rejects backing with an external Python data client. LEAN has synthetic historical/forward H1 strategy and risk parity, pending-order setup, and controlled running-engine delivery recovery. A direct negative-fill correction leaves running LEAN inconsistent. A mock snapshot revision arriving during restart requires a before/after barrier; fresh setup from a stable corrected snapshot matches both orders and account. Neither engine is adopted. Start at `docs/evidence/TL-001A-ENGINE-DECISION-MAP.md`, then `docs/evidence/TL-001A-LEAN-SNAPSHOT-RESTART-RACE.md` and `scripts/lean_fallback/`.
