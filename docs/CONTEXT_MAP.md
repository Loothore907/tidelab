...
- [Issue #2](https://github.com/Loothore907/tidelab/issues/2): TL-001A engine evaluation. Pinned NautilusTrader 2.0.0rc5 rejects backing with an external Python data client. LEAN has synthetic historical/forward H1 strategy and risk parity, pending-order setup, and controlled running-engine delivery recovery. A synthetic correction journal with two orders replays across processes, but a direct negative-fill correction leaves running LEAN inconsistent; fresh setup from the corrected mock account matches. Neither engine is adopted. Start at `docs/evidence/TL-001A-ENGINE-DECISION-MAP.md`, then `docs/evidence/TL-001A-LEAN-CORRECTION-JOURNAL.md` and `scripts/lean_fallback/`.
