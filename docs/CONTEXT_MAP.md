...
- [Issue #2](https://github.com/Loothore907/tidelab/issues/2): TL-001A engine evaluation. Pinned NautilusTrader 2.0.0rc5 rejects backing with an external Python data client. LEAN has synthetic historical/forward H1 strategy and risk parity, pending-order setup, and controlled running-engine delivery recovery. A test-only, single-writer paper source serializes correction intake and conditional mock submission. A separate durable-intent probe recovers controlled before-dispatch and lost-acknowledgement windows by stable client ID, while unknown and conflicting reports hold. It is not joined to LEAN's restored order/account state; neither engine is adopted. Start at `docs/evidence/TL-001A-ENGINE-DECISION-MAP.md`, then `docs/evidence/TL-001A-LEAN-PAPER-INTENT-RECOVERY.md` and `scripts/lean_fallback/`.
